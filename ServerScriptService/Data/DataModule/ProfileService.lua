...
local AutoSaveListLookup = {} -- [profile] = index
local MAX_ISSUE_QUEUE_SIZE = 100
...
local function RegisterIssue(error_message, store_name, store_scope, profile_key)
	...
	if #IssueQueue > MAX_ISSUE_QUEUE_SIZE then
...
local function RemoveProfileFromAutoSave(profile)
	local auto_save_index = AutoSaveListLookup[profile]
	if auto_save_index ~= nil then
		table.remove(AutoSaveList, auto_save_index)
		AutoSaveListLookup[profile] = nil

		-- Update indices
		for i = auto_save_index, #AutoSaveList do
			AutoSaveListLookup[AutoSaveList[i]] = i
		end
...
local function AddProfileToAutoSave(profile)
	...
	-- ?? Update lookup table
	for i = AutoSaveIndex, #AutoSaveList do
		AutoSaveListLookup[AutoSaveList[i]] = i
	end
...
table.insert(ModuleConnections, heartbeatConnection)
...
-- ?? Cleanup function
local function CleanupProfileService()
	...
	table.clear(AutoSaveList)
	table.clear(AutoSaveListLookup)
	table.clear(ActiveProfileStores)

	-- Cleanup write queue
	for store, keys in pairs(CustomWriteQueue) do
		for key, data in pairs(keys) do
			if data.CleanupJob and data.CleanupJob.Connected then
				data.CleanupJob:Disconnect()
			end
		end
	end
	table.clear(CustomWriteQueue)
end

ProfileService._Cleanup = CleanupProfileService
